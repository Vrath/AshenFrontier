...
**Done when:**
- [ ] Enemy grid exists with hardcoded enemies placed in slots
- [ ] Heroes target front row enemies first
- [ ] Dead units are removed from their slot
- [ ] Combat ends when one side has no units remaining
- [ ] A result message shows (win / lose)
...
**One phase at a time.** Seriously. The GDD exists. Trust it. Don't design 
during build phases.

**Data before UI.** Every feature starts as an object and a function. 
The visual comes after.

**If you're bored, you're probably in the right phase.** The interesting 
parts come later. The boring foundation is what makes them possible.

**Commit after every phase.** Even if the code is messy. Especially if 
the code is messy. You need a restore point.